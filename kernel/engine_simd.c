...
the inner loop of matmul is a dot product. with AVX2, we load 8 floats at
once into a 256-bit register and use FMA to accumulate. this reduces the
...
so load_ps accesses sequential addresses — no cache misses mid-row.
the Weights struct and stack arrays are 32-byte aligned (see engine.h ALIGN)
so we can use aligned loads on rows whose byte length is a multiple of 32.
*/
...
// dot product using FMA on 8-wide vectors.
// 'a' and 'b' must be 32-byte aligned when n is a multiple of 8 for best
// throughput, but we use loadu_ps to handle the general case safely.
...
#else
// fallback in plain C if compiler does not have AVX2 instructions enabled
static void matmul_avx2(const float *W, const float *bias,
                         const float *in, float *out, int m, int n) {
    for (int i = 0; i < m; i++) {
        float acc = bias[i];
